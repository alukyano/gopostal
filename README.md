...
```sh
git clone https://github.com/alukyano/gopostal
cd gopostal
```
</details>
...
This section will guide you to setup a fully-functional local copy of `gopostal`
...
from [`gopostal's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `gopostal`, run the
command
...
Tests in `gopostal` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *gopostal*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *gopostal* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *gopostal* 	|     docker    	|
...
```text
gopostal_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
gopostal_v1.2.1_windows_x86_64.exe
```
...
To run `gopostal` in a docker container, read the instructions in
[docker section](./docker).

### Running `gopostal`

To run gopostal, use the command
...
`gopostal` is powered by a template generated using [`go-template`][go-template-link]
