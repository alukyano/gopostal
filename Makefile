...
.PHONY: install
## `install`: Fetch dependencies needed to run `gopostal`
install:
...
.PHONY: run
## :
## `run`: Run `gopostal` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `gopostal` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `gopostal`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `gopostal`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
