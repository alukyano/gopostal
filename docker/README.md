...
a docker containers for *gopostal*!
...
```bash
make docker-gen IMAGE=gopostal VERSION=latest
```
...
```bash
make clean-docker IMAGE=gopostal VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *gopostal* uses
...
```bash
make docker-debug IMAGE=gopostal-debug VERSION=latest
```
...
```bash
docker run gopostal-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it gopostal-debug bash
```
...
```bash
make clean-docker IMAGE=gopostal-debug VERSION=latest
```
